...
export default function Coin(props: { marketCoin: any }) {
  const {
    image,
    name,
    market_cap_rank,
  ...
  const handleMarketCap = (market_cap: number) => {
    if (market_cap > 1e12) {
  ...
  return (
    <Pressable
      ...
      onPress={() => navigation.navigate("CoinDetails" as never, { coinId: id } as never)}
    >
      <Image
